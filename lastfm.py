...
def get_or_fetch_artist_metadata(
    ...
    """
    Get artist metadata (tags & top tracks) from database cache if available,
    otherwise fetch from Last.fm and persist to database.
    """
    if not artist or not artist.strip():
        return {"artist": "", "tags": [], "top_tracks": []}
    ...
    if not force_refresh:
        ...
        if cached and (cached.get("tags") or cached.get("top_tracks")):
            return {
    ...
    if tags or top_tracks:
        try:
            database.save_artist_metadata(clean_artist, tags=tags, top_tracks=top_tracks, db_path=db_path)
        except Exception as e:
