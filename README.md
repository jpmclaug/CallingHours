...
- **Setlist.fm Live Concert & Tour Intelligence**: Integration with Setlist.fm REST API v1.0 providing live concert history, average tour setlist playlists by year with concert play frequencies and stage ordering, spotlighting when the artist last played in North Carolina (venue, city, tour, setlist link), and identifying who they played with on their last 3 tours (co-headliners, tour mates, openers, and festival lineups).
- **Multi-API Artist Intelligence Page (`/artist`)**: A comprehensive dedicated artist profile unifying data across Setlist.fm, Last.fm, TheAudioDB, Genius, and local database history. Explore band origins, formation year, artwork, biography, similar artists, launch one-click average tour setlist playlist generation for any touring year, and launch one-click Gemini thematic analysis on any song in their catalog.
...
- **Playlist Generator (`/playlists`)**: Dedicated playlist creation suite that creates custom playlists from your analyzed music catalog and live tour data. Features 6 generation modes:
  ...
  - **Average Setlist by Year (Setlist.fm)**: Generate authentic tour setlists based on real concert data from Setlist.fm for any artist and calendar year, ordered by actual concert stage sequence (opener to encore closer), displaying show play counts and percentages, and cross-referenced with your Calling Hours Gemini lyrics analyses.
