...
class TestAppIntegration(unittest.TestCase):
    ...
    def test_47_playlist_user_friendly_ux_and_loading_lock(self):
        """Verify user-friendly compact mode selector, autogenerated names, and loading lock screens."""
        # 1. Seed sample analyzed data
        database.save_search(
            artist="Deftones",
            song="Change (In the House of Flies)",
            lyrics="I watched you change",
            source="Genius",
            track_tags=[{"name": "alternative-metal"}],
            theaudiodb_data={"tempo": 110, "energy": 75},
            db_path=self.db_path
        )
        database.save_analysis(
            artist="Deftones",
            song="Change (In the House of Flies)",
            analysis="## Meaning\nAtmospheric heavy alt-rock.",
            model_name="gemini-3.8-flash",
            db_path=self.db_path
        )

        # 2. Check compact strategy selector and loading overlay data attributes on /playlists
        with self.authed_get("/playlists") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            # Compact mode selector classes and banner
            self.assertIn('class="playlist-mode-grid"', html)
            self.assertIn('class="playlist-active-banner"', html)
            self.assertIn('1. Select Generator Strategy', html)
            self.assertIn('2. Configure Generator Settings', html)
            # Default autogenerated name
            self.assertIn('value="Calling Hours: All Analyzed Tracks"', html)
            self.assertIn('Calling Hours: All Analyzed Tracks', html)
            # ↺ Auto button exists
            self.assertIn('id="btn-reset-name"', html)
            self.assertIn('↺ Auto Title', html)
            # Check loading lock attributes on all 6 strategy buttons
            self.assertIn('data-loading-title="Loading All Analyzed Tracks..."', html)
            self.assertIn('data-loading-title="Loading Artist Playlist..."', html)
            self.assertIn('data-loading-title="Loading Genre & Mood Tags..."', html)
            self.assertIn('data-loading-title="Loading Audio Radar..."', html)
            self.assertIn('data-loading-title="Loading Spotify History..."', html)
            self.assertIn('data-loading-title="Loading Tour Setlist Intelligence..."', html)
            # Check tab navigation loading lock attributes
            self.assertIn('data-loading-title="Opening Generator Studio..."', html)
            self.assertIn('data-loading-title="Loading Saved Playlists..."', html)
            # Verify JS functions exist
            self.assertIn('function updatePlaylistName(', html)
            self.assertIn('function submitGeneratorForm(', html)
            self.assertIn('function onCriteriaChange(', html)
            self.assertIn('function loadSavedPlaylist(', html)

        # 3. Verify title autogeneration updates across modes, artists, years, and tags
        # Artist mode with Deftones
        with self.authed_get("/playlists?mode=artist&artist=Deftones") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Calling Hours: Deftones (Analyzed)"', html)
            self.assertIn('Calling Hours: Deftones (Analyzed)', html)

        # Tag mode with alternative-metal
        with self.authed_get("/playlists?mode=tag&tag=alternative-metal") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Calling Hours: #alternative-metal Vibes"', html)

        # Mood mode with danceable
        with self.authed_get("/playlists?mode=mood&mood=danceable") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Calling Hours: Upbeat &amp; Danceable 💃"', html)

        # Setlist.fm mode with Deftones 2024
        with self.authed_get("/playlists?mode=setlist_fm&artist=Deftones&year=2024") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Deftones: 2024 Average Setlist"', html)
            self.assertIn('Deftones: 2024 Average Setlist', html)

        # Changing year to 2022 autogenerates updated title for 2022
        with self.authed_get("/playlists?mode=setlist_fm&artist=Deftones&year=2022") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Deftones: 2022 Average Setlist"', html)

        # Custom name override is preserved when explicitly provided
        with self.authed_get("/playlists?mode=artist&artist=Deftones&name=My+Curated+Deftones") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="My Curated Deftones"', html)

        # 4. Save playlist and verify loading lock on saved view and quick load dropdown
        save_payload = json.dumps({
            "name": "Deftones Favorites",
            "description": "Top tracks",
            "generator_type": "artist",
            "items": [{"artist": "Deftones", "song": "Change (In the House of Flies)"}]
        }).encode('utf-8')
        ...
        with urllib.request.urlopen(save_req) as s_resp:
            self.assertEqual(s_resp.status, 200)
            s_data = json.loads(s_resp.read().decode('utf-8'))
            pl_id = s_data["playlist_id"]

        # View Saved Playlists tab
        with self.authed_get("/playlists?tab=saved") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn("Deftones Favorites", html)
            # Load button has loading lock
            self.assertIn('data-loading-title="Loading Saved Playlist..."', html)
            self.assertIn('data-loading-subtitle="Deftones Favorites"', html)
            # Delete button has loading lock in confirm handler
            self.assertIn("showActionLoadingOverlay", html)
            self.assertIn("Deleting Playlist...", html)

        # Loading the saved playlist directly updates the title to the saved playlist name
        with self.authed_get(f"/playlists?id={pl_id}") as resp:
            self.assertEqual(resp.status, 200)
            html = resp.read().decode('utf-8')
            self.assertIn('value="Deftones Favorites"', html)
            self.assertIn('Deftones Favorites', html)
            # Quick load dropdown is present
            self.assertIn('id="quick-select-saved-playlist"', html)

        # Clean up saved playlist
        database.delete_saved_playlist(pl_id, db_path=self.db_path)
