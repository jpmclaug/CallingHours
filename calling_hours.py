    ...
    <script>
        ...
        function updatePlaylistName(triggerField) {
            ...
            if (mode === 'all_analyzed') {
                ...
                let artist = '';
                if (artistSelect) {
                    if (artistSelect.value === '__custom__') {
                        const customInput = document.getElementById('input-setlist-artist-custom');
                        artist = customInput ? customInput.value.trim() : '';
                    } else {
                        artist = artistSelect.value.trim();
                    }
                }
                if (!artist) {
        ...
        function onSetlistArtistChange(selectEl) {
            if (!selectEl) return;
            const customContainer = document.getElementById('setlist-custom-artist-container');
            const customInput = document.getElementById('input-setlist-artist-custom');
        ...
        function onCustomArtistSubmit() {
            ...
            if (!matched) {
                const opt = new Option(`${val} (Custom)`, val, true, true);
...
class CallingHoursRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def render_playlists_page(
        ...
        for a in analyzed_artists:
            if a.get('artist'):
                library_band_names.add(a['artist'].strip())
        for b in distinct_bands_list:
            if b.get('artist'):
        ...
        def _is_matching_artist(a1: str, a2: str) -> bool:
            ...
            if s1 == s2:
                return True
            b1 = re.sub(r'[\s\-_]+(?:617|\d{3,4}|\([^\)]+\))$', '', s1).strip()
            b2 = re.sub(r'[\s\-_]+(?:617|\d{3,4}|\([^\)]+\))$', '', s2).strip()
            return b1 == b2 or s1 == b2 or s2 == b1
        ...
        if active_mode == 'artist':
            artist_opts = "".join(
                ...
                (' selected' if _is_matching_artist(a['artist'], selected_artist) else '') +
            ...
            mode_specific_inputs = f'''
                <div class="playlist-input-group" style="position: relative;">
                    <label for="select-setlist-artist">Artist / Band</label>
                    <select name="artist" id="select-setlist-artist" onchange="onSetlistArtistChange(this)" style="width: 100%;">
                        {band_opts_html}
                    </select>
                    <div id="setlist-custom-artist-container" style="display: {'block' if is_custom_artist else 'none'}; margin-top: 8px;">
                        <div style="display: flex; gap: 6px;">
                            <input type="text" id="input-setlist-artist-custom" value="{html_escape(selected_artist if is_custom_artist else '')}" placeholder="Type band name (e.g. Jimmy Eat World)..." style="flex: 1; padding: 7px 10px; font-size: 0.85rem;" onkeydown="if(event.key==='Enter'){{event.preventDefault();onCustomArtistSubmit();}}">
                            <button type="button" onclick="onCustomArtistSubmit()" class="btn-playlist-action" style="padding: 6px 12px; font-size: 0.8rem; background: #194685; color: #fff;">Go</button>
                        </div>
                    </div>
                </div>
                <div class="playlist-input-group">
